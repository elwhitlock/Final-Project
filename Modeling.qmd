...
Classification trees are just regression trees where the response is categorical. So what is a regression tree? A regression tree INSERT
...
rf_rec <- recipe(Diabetes_binary ~ HighBP + HighChol + BMI + Age + GenHlth,
                 data = train_data)
...
# (x levels) based on our folds using log 
# loss as specified
...
final_tree_test <- last_fit(tree_final_wf, data_split, metrics = metric_set(mn_log_loss)) |>
  collect_metrics()
