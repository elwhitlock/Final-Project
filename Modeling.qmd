...
Recall based on our EDA we found the following predictors useful when diabetes status 
(Diabetes_binary) is the response of interest:
...
People with diabetes tend to rank their general health more poorly than people without diabetes.
...
We will follow the same process for reading in our data as in the EDA where we use 
mutate() and factor to get our variables as the correct type. We do not drop variables here.
...
Now we need to split the data for modeling. Per Dr. Post's request we will do a 70/30 split 
and a five fold CV. Splitting on Diabetes_binary. To ensure reproducibility I set the seed 
to 67 (my mom was born in 1967).
...
uses a series of binary splits on predictor variables to classify observations into groups. 
Splits are decided based on some impurity metric. Note classification trees are regression 
trees with a categorical response. Both are easy to interpret, since they rely on visuals.
However, they are high-variance models, meaning small changes in the data can lead to a very different tree.
Because of this I expect our random forest will perform better.
...
is considered. This randomness, in the sample and the predictors, ensures the trees are 
less correlated with one another. It aggregates the results to get final predictions. 
Because it averages many trees, a random forest has lower variance and than a single tree. 
The random forest also uses an impurity metric to decide which tree to go with, like the
classification tree.
...
Finally lets compare the two models by fitting on the training set and using the test set
to see which one we will choose.
...
final_rf_test
```
We see the mean log loss is slightly lower for the random forest, .329 versus .331, so that will we
our final model!