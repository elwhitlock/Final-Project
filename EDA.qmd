...
I want to create a histogram looking into the distribution of BMI by diabetes status, I will also facet by sex to see if that impacts our results.
...
Here we see that the heavy drinking group is smaller than the not heavy drinking group for both those
with diabetes and those without. However, we we see that the smoker group is larger for the diabetes group. 
Lets try combining the two into a bar plot that displays proportion to see if this reveals any patterns.
...
ggplot(db_data, aes(x = Diabetes_binary, fill=HvyAlcoholConsump)) +
  ...
  labs(title = "Proportion of Diabetes", x = "Diabetes Status", y = "Count", fill = "Alcohol Consumption")
```

This communicates to us that people with diabetes do not have a higher proportion of
heavy drinkers who smoke than those without diabetes. Maybe these two variables are not
useful in predicting diabetes status.

From my knowledge, I know cholesterol and blood pressure are 
